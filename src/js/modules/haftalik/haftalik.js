...
import { isHoliday, isArife, getHolidayName } from '../../utils/holidays.js';
...
import { logActivity }                    from '../../utils/analytics.js';
...
function _render() {
  ...
  const portal = document.getElementById('haftalik-toolbar-portal');
  if (portal) {
    const adminButtons = _isAdmin() ? `
      <button class="btn btn--ghost btn--sm" id="btn-export-excel" type="button" aria-label="Excel olarak indir">
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true" style="margin-right:4px">
          <path d="M9 2H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V6L9 2z"/>
          <path d="M9 2v4h4"/>
          <path d="M6 10l4-4M6 6l4 4" stroke-linecap="round"/>
        </svg>
        Excel
      </button>
    ` : '';
...
function _tableHTML() {
  ...
  const headCells = days.map((dateStr, i) => {
    ...
    const arife   = isArife(dateStr);
    const holName = getHolidayName(dateStr);
    const dayDate = parseDate(dateStr);

    const thCls = hol ? 'th-holiday' : arife ? 'th-arife' : '';
    return `<th class="${thCls}">
  ...
  const bodyRows = visiblePersons.map(p => {
    ...
    const dayCells = days.map((dateStr, i) => {
      ...
      return `<td class="day-cell ${isToday ? 'col-today' : ''} ${isHol ? 'col-holiday' : ''} ${isArifeDay ? 'col-arife' : ''}" ${dropAttr}>
...
function _bindToolbarEvents() {
  document.getElementById('btn-week-prev')?.addEventListener('click', () => {
    ...
    const badge = document.querySelector('.toolbar-week-badge');
    if (badge) badge.textContent = parseInt(_currentWeek.split('-W')[1], 10);
    _renderTable();
  });
  ...
  document.getElementById('btn-export-excel')?.addEventListener('click', _exportExcel);
}
  ...
  ROOT.querySelectorAll('[data-del-person]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      _showConfirm(`${entry?.desc || 'Bu kayıt'} silinsin mi?`, () => {
        logActivity(_activeUser(), 'entry_delete', `${entry?.firma || ''} — ${entry?.urun || ''}`);
        const d = _dataFor(_currentWeek);
        const ck = `${pid}_${di}`;
        if (d[ck]) {
          d[ck].splice(ei, 1);
    ...
    chip.addEventListener('mouseenter', () => {
      _chipTipTimer = setTimeout(() => {
        try {
          ...
          const lines = [
            d.count ? `<span class="chip-hover-tip__count">${_esc(String(d.count))} araç</span>` : '',
            d.firma ? `<span>${_esc(d.firma)}</span>` : '',
            d.urun  ? `<span>${_esc(d.urun)}</span>`  : '',
            d.desc  ? `<span class="chip-hover-tip__desc">${_esc(d.desc)}</span>` : '',
          ].filter(Boolean).join('');
          if (!lines) return;
...
function _saveEntry() {
  ...
  const isEdit = _modalCtx.editIdx !== null;
  logActivity(_activeUser(), isEdit ? 'entry_edit' : 'entry_add', `${firma} — ${urun}`);
...
function _toggleComplete() {
  ...
  const entry  = d[ck][_modalCtx.editIdx];
  const action = entry.completed ? 'entry_complete' : 'entry_activate';
  logActivity(_activeUser(), action, `${entry.firma || ''} — ${entry.urun || ''}`);
  _closeModal();
  _renderTable();
  _save();
}
...
async function _openMonthly() {
  ...
  // Ayın tüm haftalarını belleğe yükle
  await _ensureMonthDataLoaded(_monthlyYear, _monthlyMonth);
  _renderMonthlyTable();
}

async function _ensureMonthDataLoaded(year, month) {
  // Ayın ilk ve son günü
  const firstDay = new Date(year, month, 1);
  const lastDay  = new Date(year, month + 1, 0);

  // İlk günün pazartesisinden başla, son güne kadar hafta hafta ilerle
  const cur = _getMonday(firstDay);
  const fetches = [];

  while (cur <= lastDay) {
    const wk = getWeekId(new Date(cur));
    if (!_data[wk]) {
      fetches.push(
        loadWeeklyData(wk).then(fresh => {
          if (fresh) _data[wk] = fresh;
          else       _data[wk] = {};
        })
      );
    }
...
function _bindMonthlyEvents() {
  ...
  document.getElementById('btn-monthly-prev')?.addEventListener('click', async () => {
    _monthlyMonth--;
    if (_monthlyMonth < 0) { _monthlyMonth = 11; _monthlyYear--; }
    await _ensureMonthDataLoaded(_monthlyYear, _monthlyMonth);
    _renderMonthlyTable();
  });
  document.getElementById('btn-monthly-next')?.addEventListener('click', async () => {
    _monthlyMonth++;
...
function _exportExcel() {
  if (typeof XLSX === 'undefined') {
    if (window.App?.showToast) {
      window.App.showToast({ title: 'Excel kütüphanesi yüklenemedi', type: 'error' });
    }
    return;
  }
  ...
  const TYPE_LABELS_MAP = { imo: "IMO'lu", imosuz: "IMO'suz", ithalat: 'İthalat', other: 'Diğer' };

  // Başlık satırı
  const dayHeaders = days.map((dateStr, i) => {
    const dt = parseDate(dateStr);
    return `${DAY_NAMES_SHORT[i]} ${dt.getDate().toString().padStart(2,'0')}.${(dt.getMonth()+1).toString().padStart(2,'0')}`;
  });
  const header = ['KİŞİ', ...dayHeaders, 'TOPLAM'];

  // Veri satırları
  const rows = visiblePersons.map(p => {
    let personTotal = 0;
    const dayCells = days.map((_, i) => {
      const entries = d[`${p.id}_${i}`] || [];
      if (!entries.length) return '';
      return entries.map(e => {
        const cnt = parseInt(e.count) || 0;
        personTotal += cnt;
        const type  = TYPE_LABELS_MAP[e.type] || e.type || '';
        const parts = [
          cnt ? `${cnt} araç` : '',
          e.firma || '',
          e.urun  || '',
    ...
    return [p.name, ...dayCells, personTotal || ''];
  });
  ...
  const totalCells = days.map((_, i) => {
    let t = 0;
    _persons.forEach(p => {
      (d[`${p.id}_${i}`] || []).forEach(e => { t += parseInt(e.count) || 0; });
    });
    return t || '';
  });
  const grandTotal = totalCells.reduce((s, v) => s + (parseInt(v) || 0), 0);
  const totalRow = ['TOPLAM', ...totalCells, grandTotal || ''];
  ...
  // Satır yükseklikleri (içerik olan satırlar için)
  ws['!rows'] = wsData.map((row, ri) => {
    if (ri === 0) return { hpt: 20 };
    const maxLines = Math.max(...row.slice(1, -1).map(cell =>
      typeof cell === 'string' ? cell.split('\n').length : 1
    ));
    return { hpt: Math.max(18, maxLines * 16) };
  });

  // Hücrelerde text-wrap
  ...
  const sheetName = formatWeekTitle(_currentWeek).replace(/[:\\/?*\[\]]/g, '-').slice(0, 31);
  XLSX.utils.book_append_sheet(wb, ws, sheetName);

  const fileName = `Haftalik-Plan_${_currentWeek}.xlsx`;
  XLSX.writeFile(wb, fileName);

  if (window.App?.showToast) {
    window.App.showToast({ title: `${fileName} indirildi`, type: 'success' });
  }
}
