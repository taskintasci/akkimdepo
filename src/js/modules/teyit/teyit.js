...
import { isHoliday, isArife }                       from '../../utils/holidays.js';
...
function _vehicleCardHTML(item, idx) {
  ...
  const createdTime = item.createdAt
    ...
    : '';
  const metaHtml = [
    item.createdBy  ? `Giriş: ${_esc(item.createdBy)}${createdTime ? ' · ' + createdTime : ''}` : '',
  ...
  return `
    <div class="vehicle-card ${cardCls} stagger animate-fade-up">
      ...
      <div class="vehicle-card__body">
        ...
        ${metaHtml ? `<div class="vehicle-card__meta">${metaHtml}</div>` : ''}
      </div>
  ...
  secimEl.innerHTML = `
    <div class="haftalik-cek-list">
      ${matched.map((e, i) => {
        ...
        const desc  = e.desc ? `<span class="haftalik-cek-item__desc">${_esc(e.desc)}</span>` : '';
        return `<button class="haftalik-cek-item" data-cek-idx="${i}" type="button">${_esc(label)}${desc}</button>`;
      }).join('')}
    </div>
  `;
